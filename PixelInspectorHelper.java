...
public class PixelInspectorHelper {

	public static Frame2D readFromURL(String url) throws IOException {
		BufferedImage bi = ImageIO.read(new URL(url));
		Frame2D frame = new MutableFrame2D(bi.getWidth(), bi.getHeight());
		for (int x=0; x<bi.getWidth(); x++) {
			for (int y=0; y<bi.getHeight(); y++) {
				frame.setPixel(x, y, rgbToPixel(bi.getRGB(x, y)));
			}
		}
		return frame;
	}
